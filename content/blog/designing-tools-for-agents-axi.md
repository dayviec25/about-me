...
Then you measure it. I benchmarked the different ways of accessing GitHub for the **exact same tasks**, and the MCP server came out badly:
...
[AXI](https://github.com/dayviec25/axi) is a set of design standards I built after noticing how much upside there is in designing tools *for agents* instead of bolting an agent onto tools built for humans. It's **10 principles** for agent ergonomics — making a tool efficient and pleasant for an agent to use, the same way we obsess over ergonomics for human UIs.
...
I also built a **Chrome DevTools AXI** and benchmarked it against other browser-automation tools. Same story: the agent took **fewer turns and used fewer tokens** to finish the same tasks. Less back-and-forth, less budget burned, same outcome.

Browser tools are notoriously expensive for agents — a single page can dump a wall of DOM into the context window. A tool designed around what the agent actually needs to *decide its next move*, rather than echoing everything, wins on every axis that matters.

These tools are free and open source. If you want to use them, the catalog and per-tool setup instructions live in [the AXI repo](https://github.com/dayviec25/axi).
